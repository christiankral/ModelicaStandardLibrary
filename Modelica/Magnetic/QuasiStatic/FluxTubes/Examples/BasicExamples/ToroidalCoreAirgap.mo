...
model ToroidalCoreAirgap "Educational example: iron core with airgap"
  ...
  Electrical.QuasiStatic.SinglePhase.Sources.VariableCurrentSource
                                                 currentSource
                    annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=90,
        origin={-50,0})));
  Blocks.Sources.Constant const(k=1/0.015/2/Modelica.Constants.pi)
  ...
  ComplexBlocks.Sources.ComplexRampPhasor complexRamp(
    ...
    phi=0,
    startTime=0.01,
    duration=0.015) annotation (Placement(transformation(extent={{-100,-30},{-80,-10}})));
  Electrical.QuasiStatic.SinglePhase.Basic.Ground
                                          electricGround2
    annotation (Placement(transformation(extent={{50,-30},{70,-10}})));
  Electrical.QuasiStatic.SinglePhase.Sensors.VoltageSensor voltageSensor annotation (Placement(transformation(
        extent={{10,-10},{-10,10}},
        rotation=90,
        origin={70,0})));
equation
  connect(core.port_n, measuringCoil.port_p)
    annotation (Line(points={{22,30},{32,30},{32,10}}, color={255,127,0}));
  connect(measuringCoil.port_n, airGap.port_p)
    annotation (Line(points={{32,-10},{32,-50},{22,-50}},
                                                 color={255,127,0}));
  connect(excitingCoil.port_n, magFluxSensor.port_n)
    annotation (Line(points={{-8,-10},{-8,-20}},   color={255,127,0}));
  connect(magFluxSensor.port_p, magneticGround.port)
    annotation (Line(points={{-8,-40},{-8,-50}},   color={255,127,0}));
  connect(magneticGround.port, airGap.port_n)
    annotation (Line(points={{-8,-50},{2,-50}},    color={255,127,0}));
  connect(excitingCoil.port_p, core.port_p)
    annotation (Line(points={{-8,10},{-8,30},{2,30}},     color={255,127,0}));
  connect(currentSource.pin_p, electricGround1.pin) annotation (Line(points={{-50,-10},{-40,-10}}, color={85,170,255}));
  connect(electricGround1.pin, excitingCoil.pin_n) annotation (Line(points={{-40,-10},{-28,-10}}, color={85,170,255}));
  connect(currentSource.pin_n, excitingCoil.pin_p) annotation (Line(points={{-50,10},{-28,10}}, color={85,170,255}));
  connect(const.y, currentSource.f) annotation (Line(points={{-79,20},{-70,20},{-70,6},{-62,6}}, color={0,0,127}));
  connect(complexRamp.y, currentSource.I) annotation (Line(points={{-79,-20},{-70,-20},{-70,-6},{-62,-6}}, color={85,170,255}));
  connect(measuringCoil.pin_n, electricGround2.pin) annotation (Line(points={{52,-10},{60,-10}}, color={85,170,255}));
  connect(electricGround2.pin, voltageSensor.pin_n) annotation (Line(points={{60,-10},{70,-10}}, color={85,170,255}));
  connect(voltageSensor.pin_p, measuringCoil.pin_p) annotation (Line(points={{70,10},{52,10}}, color={85,170,255}));
